...
\item Set a limit in the prompt: ``use a 200k token budget.'' The limit is hard.
\item Workflows run in the background. Type \texttt{/workflows} to watch.
\item If the run stops, resume the session. Completed agents are not run again.
\item Press \texttt{s} in the workflow menu to save a workflow. Share it as a skill.
\item Pair with \texttt{/loop} for repeated tasks and \texttt{/goal} for a hard finish line.
\item Most tasks do not need a workflow. Ask: does this task really need more compute?
\end{itemize}
...
{\footnotesize\color{graymid} Why this works: one context window must plan and do the work. Long tasks cause laziness, self-preference, and goal drift. A workflow gives each agent one small goal and a clean context.}
